...
def load_and_prepare_data(historical_csv, scheduled_csv=None, budget_csv=None):
    ...
    # Convert date, stripping time if present
    hist_df['date'] = pd.to_datetime(hist_df['date'])
    hist_df['day_of_week'] = pd.to_datetime(hist_df['date']).dt.dayofweek
    
    # Aggregate to daily totals (in case of duplicates within a day)
    hist_ts = hist_df.groupby(['date', 'location', 'modality'])['exam_count'].sum().reset_index()
    hist_ts = hist_ts.rename(columns={'date': 'ds', 'exam_count': 'y'})
    
    # Calculate historical capacity
    capacity_df = hist_ts.groupby(['location', 'modality'])['y'].max().reset_index()
    capacity_df = capacity_df.rename(columns={'y': 'max_capacity'})
    print("\nCalculated Daily Historical Capacities:")
    print(capacity_df)
...
def predict_future_exams(models, start_date, days_ahead, scheduled_df, budget_df, capacity_df, dow_effects):
    """Predict exam counts using Prophet models"""
    if not models:
            ...
            for _, row in day_scheduled.iterrows():
                ...
                if model and last_train_date:
                    ...
                    if days_ahead > 0:
                        ...
                        forecast = model.predict(future)
                        pred_additional_raw = max(0, int(round(forecast['yhat'].iloc[-1])))
